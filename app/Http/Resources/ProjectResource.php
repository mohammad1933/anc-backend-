...
class ProjectResource extends JsonResource
{
    ...
    {
        $coverImageUrl = $this->imageUrl($this->cover_image);
        $inspirationImageUrls = collect($this->inspiration_images ?? [])
            ->map(fn ($path) => $this->imageUrl($path))
            ->all();

        return [
            'id' => $this->id, 'name' => $this->name, 'client' => $this->client, 'description' => $this->description,
            'cover_image_url' => $coverImageUrl, 'cover_image' => $coverImageUrl, 'status' => $this->status,
            ...
            'notes' => $this->notes ?? [], 'inspiration_image_urls' => $inspirationImageUrls, 'inspiration_images' => $inspirationImageUrls,
